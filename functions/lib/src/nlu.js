...
exports.normPhone = void 0;
exports.parseQuantityPhrase = parseQuantityPhrase;
exports.detectIntent = detectIntent;
exports.stripPriceNoise = stripPriceNoise;
    ...
    for (const segRaw of segments) {
        ...
        if (leadWord && leadWord[1] in NUM_WORDS) {
            ...
            if (withTail) {
                ...
                if (bare[2]) {
                    if (UNIT_WORDS.includes(bare[2]))
                        unit = singular(bare[2]);
                    else
                        rest = bare[2];
                }
                leadingParsed = true;
        ...
        if (trailX) {
            ...
            if (!leadingParsed &&
                ...
                if (trailNum[2] && UNIT_WORDS.includes(trailNum[2]))
                    unit = singular(trailNum[2]);
...
function detectIntent(raw) {
    ...
    if (!t)
        return 'UNKNOWN';
    if (/^(hi|hii+|hello|hey|namaste|good\s?(morning|evening|afternoon)|start|menu)\b/.test(t))
        return 'GREETING';
    if (/^[1️⃣]|^1$/.test(t))
        return 'MENU_1_ORDER';
    if (/^[2️⃣]|^2$/.test(t))
    ...
    if (/^[3️⃣]|^3$/.test(t))
        return 'MENU_3_INFO';
    if (/^[4️⃣]|^4$/.test(t))
        return 'MENU_4_CONTACT';
    if (/\b(remove|delete|drop)\b/.test(t))
        return 'REMOVE_ITEM';
    if (/\b(cart|what do i have|my order details)\b/.test(t) && !/\bcheckout\b/.test(t))
        return 'VIEW_CART';
    ...
    if (/\bconfirms?\b|\bconfirmed\b/.test(t))
        return 'CONFIRM';
    if (/\b(checkout|place the order|pay now|proceed)\b/.test(t))
        return 'CHECKOUT';
    if (/^(yes|yep|yeah|ok|okay|sure|haan|ji haan)\b/.test(t))
        return 'YES';
    if (/^(no|nope|nahi|cancel it|cancel)\b/.test(t))
        return t.includes('cancel') ? 'CANCEL' : 'NO';
    if (/\b(track|where is my order|order status|status of)\b/.test(t) || /^#?ord-?\d+$/i.test(t))
        return 'TRACK';
    ...
    // price questions.
    if (/\b(do you have|have you got|in stock|available|availability)\b/.test(t) &&
        !/\b(add|i want|i need|give me)\b/.test(t) &&
        !/\b(price|cost|how much|rate)\b/.test(t))
        return 'STOCK_QUERY';
    if (/\b(price|cost|how much|rate)\b/.test(t) && !/\b(add|i want|i need|give me)\b/.test(t))
        return 'PRICE_QUERY';
    if (/\b(add|want|need|get me|give me|looking for|do you have|have you got|order)\b/.test(t))
        return 'ADD_ITEMS';
