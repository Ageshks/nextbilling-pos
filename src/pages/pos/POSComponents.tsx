    ...
    <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800/60">
      <div className="flex shrink-0 items-center justify-between border-b border-slate-200 px-3 py-2.5 dark:border-slate-700">
        <div className="flex items-center gap-2">
          <ShoppingCart className="h-4 w-4 text-slate-500 dark:text-slate-400" aria-hidden="true" />
          <h2 className="text-sm font-semibold text-slate-900 dark:text-white">Current sale</h2>
        </div>
        <Badge tone="slate" className="tabular-nums">
          {formatNumber(cart.itemCount, 2)} {cart.itemCount === 1 ? 'item' : 'items'}
        </Badge>
      </div>

      {/* This is intentionally the only scrollable region in the cart panel. */}
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-3 pr-2">
        {cart.items.length === 0 ? (
          <div className="flex h-full min-h-28 flex-col items-center justify-center py-8 text-center text-sm text-slate-500">
            <ShoppingCart className="mx-auto h-6 w-6 text-slate-300" aria-hidden="true" />
            <p className="mt-2 font-medium">Cart is empty</p>
            <p className="mt-1 text-xs">Add products to start a new sale.</p>
          </div>
        ) : (
      ...
      {/* Summary and checkout deliberately sit outside the scrollable item list. */}
      <div className="shrink-0 space-y-2 border-t border-slate-200 p-3 dark:border-slate-700">
        <BillDiscountInput
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="max-w-52 truncate font-medium text-slate-800 dark:text-slate-200">{cart.customer.name}</span>
        </div>
        ...
        <Button className="w-full" size="lg" leftIcon={<ReceiptText className="h-5 w-4" />} onClick={onPayment} disabled={cart.itemCount === 0 || receiving}>
          Payment (F8 / F10)
        </Button>

        {canCash && session ? (
          <Button className="w-full" size="sm" variant="outline" leftIcon={<LogOut className="h-4 w-4" />} onClick={onCloseShift}>
            Close shift
          </Button>
        ) : null}
      </div>
